...
export const createPrediction = createAsyncThunk(
    ...
    async ({ patientId, medicalData, doctorId }, { rejectWithValue }) => {
        try {
            console.log("API Request: POST /predictions", { patientId, medicalData, doctorId })

            const response = await predictionsAPI.post("/api/predictions", {
                patientId,
                medicalData,
                doctorId
            })
            ...
            return rejectWithValue(error.response?.data?.error || "Failed to create prediction")
        }
    }
)
...
export const fetchPredictions = createAsyncThunk(
    ...
    async ({ doctorId }, { rejectWithValue }) => {
        try {
            const response = await predictionsAPI.get(`/api/predictions?doctorId=${doctorId}`)
            return response.data.data
        } catch (error) {
...
export const fetchPredictionHistory = createAsyncThunk(
  ...
  async (patientId, { rejectWithValue }) => {
    try {
      ...
      return rejectWithValue(error.response?.data?.message || "Failed to fetch prediction history")
    }
  },
)
